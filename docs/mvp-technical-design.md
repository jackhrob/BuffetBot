...
Proposed September 7, 2026. This document describes the target architecture. [BB-001](development/01-project-foundation.md) provides the verified local Python/configuration foundation, [BB-002](development/02-engine-qualification.md) qualifies the engine with synthetic experiments, [BB-003](development/03-contracts-and-experiments.md) implements the shared contracts, and [BB-004](development/04-dataset-snapshots.md) implements local market snapshots and offline fixtures. The [BB-005 adapter/cache](market-data.md) passes synthetic tests; actual provider verification is blocked by missing credentials. [BB-006](jobs.md) implements the local durable research-job lifecycle and single-worker process boundary. Production trading, model, and broker integration work remains planned. It refines the earlier [build proposal](build-proposal.md), particularly the engine choice and local model deployment.
